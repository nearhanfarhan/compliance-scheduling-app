...
try:
    # Service account and Maps API Key for the backend
...
if 'token' not in st.session_state:
    ...
    result = oauth2.authorize_button(
        ...
        redirect_uri="https://compliance-scheduling.streamlit.app/",
    ...
    # If logged in, get the user's email and info
    token = st.session_state['token']
    user_info = token.get('userinfo')
    user_email = user_info.get('email')

    # Check if the email domain is correct
    if user_email.endswith('@li.me'):
        ...
        # ... (All other helper functions: generate_worker_availability, generate_reliable_cyclists, etc., are unchanged)
